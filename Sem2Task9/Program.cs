...
//Вариант 1
 System.Random numSintezator = new System.Random(); //new выделяет опертиву под объект

// int rndNum = numSintezator.Next(10, 100); // задаем случайное число
// Console.WriteLine("Случайное число: " + rndNum);

// int digit1 = rndNum / 10; //мы хотим увидеть это число
// int digit2 = rndNum % 10; //здесь нам нужен отстаток от деления на 10
...
//Вариант 2
// char[] digits = numSintezator.Next(10,100).ToString().ToCharArray();  //создаем массив = вызываем создание числа
// Console.WriteLine(digits);
// int digit1 = (int)digits[0]-48; // -48 это преобразование из вещественного числа
// int digit2 = (int)digits[1]-48; 
// int resNum = digit1>digit2?digit1:digit2;  //в resNum мы положим самое большое и чисел, но воспользуемся тернарным оператором
// Console.WriteLine(resNum);

// //Вариант 3 
int rndNum = numSintezator.Next(10, 100); //берем число котрое будет заполнено
Console.WriteLine(rndNum); // число будет рандомом и будет показыавать лишь адрес ячейки
int[] indxAnsw = new int[90]; //в массиве заполняем все возможные варианты, заполняем готовыми ответами
...
Console.WriteLine(indxAnsw[rndNum-10]);  //так числа с 10 до 99, а нумерация в массиве с 0, делаем - 10. т.е 10 будет в 0 массиве
